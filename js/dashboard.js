...
function populateFilter2() {
  ...
  filter2.innerHTML =
    "Select a period (in years, more than 1 and no more than 10): ";
      ...
      if (
        ...
        if (parseInt(filter2Data) > 1 && parseInt(filter2Data) < 11) {
          ...
          if (data != null && data != undefined && data != 0) {
            ...
            filterButtonC.innerHTML = "No data received, try again.";
          }
        } else {
          filterButtonC.innerHTML = "Invalid filter inputs, try again.";
        }
      } else {
        filterButtonC.innerHTML = "Invalid filter inputs, try again.";
      }
...
function applyDashBGraph0(data) {
  ...
  for (let i = 0; i < data[1].length; i++) {
    yLabel[i] = data[1][i].Year;
    yTotal[i] = data[1][i].All;
    dataSet[i] = [
      data[1][i].Online,
      data[1][i].Buses,
      data[1][i]["Region 1"],
    ...
    for (let j = 0; j < xLabel.length; j++)
      if (dataSet[i][j] == null) dataSet[i][j] = 0;
    dataPack[i] = {
      label: yLabel[i],
      data: dataSet[i],
      borderColor: yColor[i],
      backgroundColor: yColor[i],
    };
  }
  ...
  for (let i = 0; i < xLabel.length; i++) {
    for (let j = 0; j < data[1].length; j++) {
      if (data[1][j].period == xLabel[i]) {
        tot[i] = data[1][j].total;
        dir0[i] = data[1][j].direction0;
        dir1[i] = data[1][j].direction1;
      }
    }
...
function applyDashBGraph6(data) {
  ...
  var dir1 = [];
  for (let i = 0; i < data[1].length; i++) {
    if (data[1][i].period == "Daily")
      yDaily = [data[1][i].total, data[1][i].direction0, data[1][i].direction1];
  }
  for (let i = 0; i < xLabel.length; i++) {
    for (let j = 0; j < data[1].length; j++) {
      if (data[1][j].period == xLabel[i]) {
